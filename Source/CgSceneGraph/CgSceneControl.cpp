    ...
    if(e->getType() & Cg::CgKeyEvent)
    {
        ...
        if (ev->text() == "e") {
            if (!entity_group_selected) {
                ...
                iterateChildren(selected_entity, glm::vec4(237.0, 36.0, 207.0, 1.0));
                m_renderer->redraw();
            } else {
                entity_group_selected = false;
                selected_entity->getAppearance().setObjectColor(glm::vec4(0.0, 255.0, 0.0, 1.0));
                iterateChildren(selected_entity, glm::vec4(0.0, 255.0, 0.0, 1.0));
                m_renderer->redraw();
            }
...
void CgSceneControl::iterateChildren(CgSceneGraphEntity* entity,glm::vec4 color) {
    for(unsigned int i = 0; i < entity->getChildren().size(); i++) {
        entity->getChildren()[i]->getAppearance().setObjectColor(color);
        iterateChildren(entity->getChildren()[i],color);
    }
